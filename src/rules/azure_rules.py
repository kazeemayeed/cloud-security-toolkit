...
class AzureRules:
    ...
    def _check_storage_public_access(self, content: Dict[str, Any]) -> List[Dict[str, Any]]:
            ...
            for resource_type, resources in content['resource'].items():
                if resource_type == 'azurerm_storage_account':
                    for resource_name, resource_config in resources.items():
                        allow_public_access = resource_config.get('allow_blob_public_access', True)
        ...
        if 'resource' in content:
            for resource_type, resources in content['resource'].items():
                if resource_type == 'azurerm_network_security_rule':
                    for resource_name, resource_config in resources.items():
                        source_address_prefix = resource_config.get('source_address_prefix', '')
    ...
    def _check_vm_disk_encryption(self, content: Dict[str, Any]) -> List[Dict[str, Any]]:
                ...
                if resource_type == 'azurerm_linux_virtual_machine' or \
                   resource_type == 'azurerm_windows_virtual_machine':
                    for resource_name, resource_config in resources.items():
                        ...
                        if not encryption_settings or not encryption_settings.get('enabled', False):
                            violations.append({
